...
class InspectionList extends React.Component {	
	...
	static async getInitialProps({ store, isServer, pathname, query, asPath, req, res }) {
		...
		// const fieldGroupsReq = await fetch(publicRuntimeConfig.api_base + 'acf-field-group?per_page=100')	   
	 //    const errorCode = fieldGroupsReq.statusCode > 200 ? fieldGroupsReq.statusCode : false;
	 //    const fieldGroups = await fieldGroupsReq.json(); 	 
	    
	    const fieldGroupsReq = await fetch(publicRuntimeConfig.api_base + 'property_inspections?slug=' + req.params.slug)	   
	    const errorCode = fieldGroupsReq.statusCode > 200 ? fieldGroupsReq.statusCode : false;
	    const fieldGroups = await fieldGroupsReq.json(); 	 
	    
		let fieldGroupData = fieldGroups[0].acf
		...
		for(var property in fieldGroupData){
			...
			if(!property.includes('inspect')){				
				...
				for(var section_question in fieldGroupData[property]){					
					...
					// if(typeof allListItems[property] == 'undefined'){
					// 	allListItems[property] = []
					// }

					// allListItems[property].push(questionData)	
